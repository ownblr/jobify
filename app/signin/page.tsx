    ...
    try {
      const res = await fetch('/api/login', {
        ...
        if (res.ok) {
          ...
          if (accountData.user.userType === 'professional') {
            router.push('/professional');
          } else if (accountData.user.userType === 'employer') {
            router.push('/employer');
          } else if (accountData.user.userType === 'staff') {
            router.push('/staff');
          }
        }
              ...
              <form onSubmit={handleSubmit}>
                <div className="grid gap-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input value={password} onChange={handlePasswordChange} type="password" id="password" name="password" className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" required aria-describedby="password-error" />
